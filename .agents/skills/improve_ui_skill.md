...
Continue until all selected changes are represented or rejected. If new findings emerge, report them for selection only after completing current plans. If the user requests, summarize all surviving findings and open plans. Do not perform acceptance testing or exploratory QA; rely only on the cited evidence.
